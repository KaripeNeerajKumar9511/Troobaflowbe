...
#define mOPRT 1
#define mOPASA 2
#define ROUT_ERR 0.0001
#define arr(i,j,n) ((i)*(n)+(j))
...
float estimate_scrap_rate(class_model* c1, class_part* tpart);
/* ################### */
...
/* ################################################ */
{
    ...
    class_oplst* toplst;
    class_oplst* f_lst;
    bool fallback_lsize = false;
    bool fallback_good = false;
...
/* add changes for STOCK and scrap */
    ...
    if ((isol != 1) || (idsol != 1)) {
        sprintf_s(buf3, "WARNING: #(21b) \n Routing lot-size equations are singular; using fallback lot-size values.");
        warn_err(c1, buf3, 1, sNULL, sNULL, tpart->name, c1->get_oper_num(tpart->oper_start)->oldname, sNULL, sNULL);
        fallback_lsize = true;
    };
    ...
    if (fallback_lsize) {
        for (i = 0;i < num;i++) mreturn[i] = tpart->lotsiz;
    }
    else {
    ...
    if (fallback_lsize) {
        // Data-driven fallback: estimate scrap from routing percentages to SCRAP.
        tpart->scrap = estimate_scrap_rate(c1, tpart);
    }
    else {
    ...
    if ((isol != 1) || (idsol != 1)) {
        sprintf_s(buf1, "WARNING: #(21c) \n Routing good-piece equations are singular; using fallback good-piece values.");
        warn_err(c1, buf1, 1, sNULL, sNULL, tpart->name, c1->get_oper_num(tpart->oper_start)->oldname, sNULL, sNULL);
        fallback_good = true;
    };
...
/* ********************** */
{
    ...
    //write_check(c1->varlocal," �n to clenup");
...
/* ********************** */
{
    ...
    class_oper* from_op;
    class_oper* to_op;
    float total_non_scrap_out = 0.0f;
    float total_to_scrap = 0.0f;
    int i_r;
    int r_start = tpart->route_start;
    int r_count = tpart->route_cnt;

    if ((r_start < 0) || (r_count <= 0)) return 0.0f;

    for (i_r = r_start; i_r < r_start + r_count; i_r++) {
        rt = c1->all_route[i_r];
        if (rt == NULL) continue;

        from_op = c1->get_oper_num(rt->op_from_ptr);
        to_op = c1->get_oper_num(rt->op_to_ptr);
        if ((from_op == NULL) || (to_op == NULL)) continue;

        // Ignore recirculation from SCRAP itself.
        if (strcmp(from_op->name, "SCRAP") == 0) continue;

        total_non_scrap_out += rt->cmmpij;
        if (strcmp(to_op->name, "SCRAP") == 0) total_to_scrap += rt->cmmpij;
    }

    if (total_non_scrap_out <= 0.0f) return 0.0f;

    // convert from % to ratio and clamp
    float s = (total_to_scrap / total_non_scrap_out);
    if (s < 0.0f) s = 0.0f;
    if (s > 0.99f) s = 0.99f;
    return s;
}
