...
#define EPS 1.0e-10
 ...
 void mpcalc   (class_model * c1);
	...
	void dif1_ci_2     (class_model * c1, float * vec, float * diff);
...
teq->y1 = 0.5 * (teq->ca2 + teq->cs2) * pow(u1, (sqrt(2.0 * (teq->num + 1.0)) - 1.0));
                //  cv * u ^ (sqrt (2*(n+1)) -1 )
...
#define FREEALL ::free_nr_vector(xi,1,n);::free_nr_vector(h,1,n);::free_nr_vector(g,1,n)
...
void linmin(class_model * c1, float p[], float xi[], int n, float *fret) //, float (*func)(float []))
{
	...
	c1->ncom=n;
	c1->pcom=::nr_vector(1,n);   //gwwd  local 9-29
	c1->xicom=::nr_vector(1,n);
	// nrfunc=func;
	for (j=1;j<=n;j++) {
		c1->pcom[j]=p[j];
		c1->xicom[j]=xi[j];
	...
	*fret=brentgd(c1, ax,xx,bx,c1->tolerance,&xmin);  //  brentgd(ax,xx,bx,f1dim,tolerance,&xmin);
	...
	::free_nr_vector(c1->xicom,1,n);  //??  gwwd ??  9-27
	::free_nr_vector(c1->pcom,1,n);   //??  gwwd ??  9-27
...
#define  MESSAGE_SIZE 400
	...
	g=::nr_vector(1,n);    //  gwwd  local 9-29
	h=::nr_vector(1,n);
	xi=::nr_vector(1,n);
...
#undef EPS
#undef FREEALL
#undef NRANSI #undef NRANSI#undef NRANSI #undef NRANSI