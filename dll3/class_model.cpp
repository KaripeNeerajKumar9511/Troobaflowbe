#include "StdAfx.h"
#include "class_model.h"   // add this
#include "class_lab.h"
#include "class_eq.h"
...
class_model::class_model(int l_size, int e_size, int p_size, int o_size, int r_size, int i_size, char * dir1)
{
	...
	// Initialize members that might be reported uninitialized by static analysis
	// (C26495 requires member initialization). Keep minimal and safe defaults.
	t1 = 0.0f;
	t2 = 0.0f;
	utlimit = 0.0f;
	v_part = v_lab = v_equip = 0.0f;
...
// setups - utilities 
	...
	class_lab *class_model::first_lab_ptr()  { 
		lab_point = 0;
			char s0 [MESSAGE_SIZE];
	...
	class_lab *class_model::next_lab_ptr()  { 
		lab_point +=1;
	...
	class_lab *class_model::xxadd_lab(char * name1, double size1)  { 
	...
	int class_model::find_lab_name( char *  lname1) { 
			int i;
	...
	class_lab * class_model::get_lab_num( int num) {
...
//    methods ....--equipment--------------
	...
	class_eq *class_model::first_eq_ptr()  { 
		eq_point = 0;
		return all_eq[0];
	};

	class_eq *class_model::next_eq_ptr()  { 
		eq_point +=1;
	...
	class_eq *class_model::first_eq_ptr2()  { 
		eq_point2 = 0;
		return all_eq[0];
	};

	class_eq *class_model::next_eq_ptr2()  { 
		eq_point2 +=1;
		if (eq_point2 >=  eq_count) return NULL;
		else return all_eq[eq_point2];
	};

	class_eq *class_model::xxadd_eq(char * name1, double size1)  { 
	...
	int class_model::find_eq_name( char * lname1) { 
			int i;
			for (i=0;i<eq_count;i++) { 
	...
	class_eq * class_model::get_eq_num( int num) {
...
//    methods ...products--
	...
	class_part *class_model::first_part_ptr()  { 
		part_point = 0;
		return all_part[0];
	};

	class_part *class_model::next_part_ptr()  { 
		part_point +=1;
	...
	class_part * class_model::first_part_ptr2 (void)  { 
		part_point2 = 0;
		return all_part[0];
	};

	class_part * class_model::next_part_ptr2 (void)  { 
		part_point2 +=1;
		if (part_point2 >=  part_count) return NULL;
		else return all_part[part_point2];
	};

	class_part *class_model::add_part(char * name1, double size1)  { 
	...
	int class_model::find_part_name( char * lname1) { 
			int i;
			for (i=0;i<part_count;i++) { 
	...
	class_part * class_model::get_part_num( int num) {
...
//    methods ....--operations--------------
	...
	class_oper *class_model::first_oper_ptr()  { 
		oper_point = 0;
		return all_oper[0];
	};

	class_oper *class_model::next_oper_ptr()  { 
		oper_point +=1;
		if (oper_point >=  oper_count) return NULL;
		else return all_oper[oper_point];
	};

	class_oper *class_model::add_oper(char * name1, int opnum)  { 
	...
	int class_model::find_oper_name( char * lname1) { 
			int i;
			for (i=0;i<oper_count;i++) { 
	...
	class_oper * class_model::get_oper_num( int num) {
...
//    methods ...route--
	...
	class_route *class_model::first_route_ptr()  { 
		route_point = 0;
		return all_route[0];
	};

	class_route *class_model::next_route_ptr()  { 
		route_point +=1;
		if (route_point >=  route_count) return NULL;
		else return all_route[route_point];
	};

	class_route *class_model::add_route( double cmmpij)  { 
	...
	class_route * class_model::get_route_num( int num) {
...
//    methods ....--ibom--------------
	...
	class_ibom *class_model::first_ibom_ptr()  { 
		ibom_point = 0;
		return all_ibom[0];
	};

	class_ibom *class_model::next_ibom_ptr()  { 
		ibom_point +=1;
		if (ibom_point >=  ibom_count) return NULL;
		else return all_ibom[ibom_point];
	};

	class_ibom *class_model::add_ibom(char * name1, double upa)  { 
	...
	class_ibom * class_model::get_ibom_num( int num) {
			char s0 [MESSAGE_SIZE];
	...
	class_wused * class_model::add_wused(void) {
		wused_point++;
	...
	void class_model::clear_model_batch(char * dir1) {
		//  set dir  save dir;
