...
// in dll2\calc1.cpp, next to the other LABOR_T_GATHER_* defines
#define LABOR_T_GATHER_1  27
#define LABOR_T_GATHER_2  28   // <-- add this line
...
// Use the correct prototype (matches class_model.h / implementation)
    ...
    for (teq = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr())
    {
        ...
        if (teq->num > 0)
        {
            ...
            if (u1 < EPSILON) teq->wait = 0.0;
                ...
                ((teq->ca2 + teq->cs2) / 2.0) *
                pow(u1, (sqrt((double)(2.0 * (teq->num + 1.0))) - 1.0)) /
                (teq->num * (1.0 - u1));
...
/*  ******************************************************************* */
{
    ...
    if (xbar2 >= (xbar1 - EPSILON)) {
        ...
        if (c1->inOPT != 0) {
            //sprintf(buf1, "in xprime %f ::  %f %f  : %f %f : %f : %f  %f ", xprime, xbar1, xbar2, xm_only, xl_only, effabs(c1, tlabor), teq->mttr, teq->mttf, tlabor->fac_eq_lab);
            //write_check(c1->varlocal, buf1);
        }
        return(xprime);
    }
...
double effabs(class_model* c1, class_lab* tlabor)
{
    ...
    if (n < 0)
        ...
        if (c1->inOPT != FALSE) {
            x = pow(tlabor->ul / 100., (double)n) * tlabor->absrate / 100;
        }
        else {
            x = pow(tlabor->ul, (double)n) * tlabor->absrate / 100;
        }
    }
...
//xxZZ
    ...
    // DOCK/SCRAP can be terminal in some legacy routing transforms.
    if ((roper == NULL)
        && (strcmp(frm_oper->name, "SCRAP") != 0)
        && (strcmp(frm_oper->name, "DOCK") != 0)) {
...
void  getx(class_eq* teq) {
    ...
    if (teq->cs2 > 1) {
        d3 = 3 * pow(teq->cs2, 1.5) * (1 + teq->cs2);
    }
    else {
