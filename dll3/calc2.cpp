...
/*  ******************************************************************* */
{
        ...
        /* ***************************** */
         ...
         for(tlabor  = c1->first_lab_ptr(); tlabor != NULL; tlabor = c1->next_lab_ptr()) {
                ...
                tlabor->xsigbar = 0.0;
				if (tlabor->num > 0) {
                   tlabor->cs2  = pow( tlabor->faccvs*c1->v_lab/100.0, 2.0);
                }
                else {
        ...
        for(tpart  = c1->first_part_ptr(); tpart != NULL; tpart = c1->next_part_ptr())
        {
                ...
                for (i_oper= o_start; i_oper< o_start+o_count; i_oper++ )                
                {
					aoper = c1->get_oper_num(i_oper);
                        ...
                        xprsig = sqrt (  2.0* pow(teq->mttr,2.0) * teq->imttf*xbar2
                            +  pow( (1.0 + teq->imttf*teq->mttr),2.0) *
                            pow( (c1->v_equip * teq->faccvs    * (xm_only)/100.0), 2.0)
                            +  tlabor->ct2 * pow( (xl_only)*(1.0 + teq->fac_eq_lab),2) );
                        ...
                        teq->xbard    += vlam1;
                        teq->xsigbar  +=
                            vlam1 * (  pow(xprsig,2.0) + pow(xprime,2.0)  );
                        ...
                        tlabor->xbard    += vlam1;
                        tlabor->xsigbar  +=
                            vlam1 * (  pow(xprsigl,2.0) + pow(xlabor,2.0)  );
         ...
         for(tlabor  = c1->first_lab_ptr(); tlabor != NULL; tlabor = c1->next_lab_ptr())
        {
                ...
                if (tlabor->num > 0) {
                       if ( tlabor->xbarbar  >  SSEPSILON)
                                tlabor->cs2  = pow( tlabor->faccvs*c1->v_lab/100.0, 0.9);
        ...
        for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr())
        {
                ...
                if(teq->num >  0)
                {
                        if ( teq->xbarbar  >  SSEPSILON)
                                teq->cs2 = (teq->xsigbar * teq->xbard / pow(teq->xbarbar,2.0)) - 1.0;
                        /*  effect of sum(l*v) / sum(l) / sum(l*x) / sum(l)**2 */
                        else
                            teq->cs2 = 1.0;
                }
                        ...
                        for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr()) {
                                if (tlabor == c1->get_lab_num(teq->lab_ptr)) {
                                        ...
                                        if ( teq->num > 0)  {
                                                ...
                                                num  = 1.0 + (teq->cs2-1.0)* rho1*rho1/(pow(s1,0.5)) -
                                                    (1.0-rho1*rho1) * (1.0-rho2*rho2) +
                                                    (1.0-rho1*rho1) * (tlabor->cs2 - 1.0)* rho2*rho2 / pow(s2, 0.5);
...
/*  ------------------------------------------- */
{
        ...
        if ( n < 0.0)
                ...
                ret = pow(u1, (pow (2.0*(n+1.0),0.5) - 1.0)) / (n * (1.0 - u1));
        else if ( n >=  m )
                ret = 0.0;
        else
                ret = pow(u1, (pow (2.0*(n+1.0),0.5) - 1.0)) / (n  * (1.0 - u1));
