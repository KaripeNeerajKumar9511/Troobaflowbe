...
class_route* find_rt_from(class_model*, char*, class_part*);
	...
	for(tpart  = c1->first_part_ptr(); tpart != NULL; tpart = c1->next_part_ptr())
	{
		...
		if ( teq->lam > SSEPSILON)
		{
			...
			if ( teq->oV > SSEPSILON)
			{
				...
				else teq->oV  =  pow(teq->lam0,2.0) / teq->oV;
				u1 = MIN(teq->u, .95);  //  gwwd  utlimit issue ??? 8-21-17
				oW     = 1.0 + 4.0*pow((1.0-u1),2.0) * (teq->oV - 1.0);
				if ( oW  > SSEPSILON)
					oW = 1.0 / oW;
				else
				{
			...
			else
			{
                ...
                if (teq->num > 0)
                        teq->X = 1.0 + pow(teq->num,-0.5)  * (MAX(0.2,teq->cs2) - 1.0);
                else
                        teq->X = teq->cs2;
                ...
                if ( teq->nV > SSEPSILON)
                {
                        teq->nV = 1.0 / teq->nV;
                        u1 = MIN(teq->u, .95); //  gwwd  utlimit issue ??? 8-21-17
                        teq->nW = 1.0 + 4.0 * pow((1.0 - u1),2.0) * (teq->nV-1.0);
        ...
        for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr())
        {
                ...
                for(teq2  = c1->first_eq_ptr2(); teq2 != NULL; teq2 = c1->next_eq_ptr2())
                {
                        ...
                        lamij[ arr(eqi,eqj,ecnt+1)] = teq->nW * ptemp * qtemp * ( 1.0 - pow(u1,2.0) );
                        ...
                        teq->A  +=  ptemp*
                            ((1.0 - qtemp) + qtemp * pow(u1,2.0) * teq2->X );
